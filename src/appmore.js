...
class Appservice extends React.Component {
    state = {
        appserviceDetails : null
    }

    async componentDidMount () {
        const appserviceDetails = await dataFunctions.getappservicesDetails();
        this.setState({appserviceDetails:appserviceDetails})
    }

    render() {
        if(this.state.appserviceDetails!=null) {
            return(
                <div>
                    {this.state.appserviceDetails.map((item,index) => (
                        <React.Fragment key={index}>
                            <div>
                                <Navbar />
                                <div className="carrer-cont">
                                    <h1 className="web-ser-title">{item.app}</h1>
                                    <p className="web-ser-inf">{item.titleappinf}</p>
