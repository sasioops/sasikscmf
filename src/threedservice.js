...
class Threedservice extends React.Component {
    state = {
        threedservicesDetails : null
    }

    async componentDidMount () {
        const threedservicesDetails = await dataFunctions.getthreedservicesDetails();
        this.setState({threedservicesDetails:threedservicesDetails})
    }

    render() {
        if(this.state.threedservicesDetails!=null) {
            return (
                <div>
                    {this.state.threedservicesDetails.map((item,index) => (
                        <React.Fragment key={index}>
                            <div>
                            <Navbar />
                                <div className="carrer-cont">
                                    <h1 className="web-ser-title">{item.thrd}</h1>
