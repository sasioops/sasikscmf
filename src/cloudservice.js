...
class Cloudservice extends React.Component {
    state = {
        cloudservicesDetails : null
    }

    async componentDidMount () {
        const cloudservicesDetails = await dataFunctions.getcloudservicesDetails();
        this.setState({cloudservicesDetails:cloudservicesDetails})
    }

    render() {
        if(this.state.cloudservicesDetails!=null) {
            return (
                <div>
                    {this.state.cloudservicesDetails.map((item,index) => (
                        <React.Fragment key={index}>
                            <div>
                            <Navbar />
                                <div className="carrer-cont">
                                    <h1 className="web-ser-title">{item.cloud}</h1>
